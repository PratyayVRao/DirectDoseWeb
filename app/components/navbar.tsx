...
export function Navbar() {
  ...
  // Define all navigation links in the correct order
  const navLinks = [
    { href: "/", label: "Home" },
    { href: "/calculator", label: "Calculator" },
  ]

  // Links for logged-in users (ICR and Basal Calculator)
  const authLinks = [
    ...
    <header className="sticky top-0 z-50 w-full bg-[#006c67] text-white shadow-md">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <nav className="hidden md:flex items-center gap-3 sm:gap-4 md:gap-6">
          ...
          {/* ICR Calculator and Basal Calculator (always show, even if not logged in) */}
          {authLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
